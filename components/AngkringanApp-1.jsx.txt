...
const Dashboard = ({orders,expenses,setExpenses,user,setScreen,target,setTarget,kasirs,businessDate,sessionOpen,sessionDate,onBuka,onTutup}) => {
  ...
  const paidToday=orders.filter(o=>o.status==="paid"&&o.paidAt===businessDate);
  ...
  const chartData=useMemo(()=>Array.from({length:7},(_,i)=>{
    ...
    return {day:d.toLocaleDateString("id-ID",{weekday:"short"}),
      total:orders.filter(o=>o.status==="paid"&&o.paidAt===ds).reduce((s,o)=>s+o.total,0),isToday:ds===businessDate};
  }),[orders,businessDate]);
...
const POS = ({menus,orders,setOrders,user,businessDate,kasirs,setScreen}) => {
  ...
  const konfirmasiBayar=()=>{
    ...
    setOrders(p=>[...p,{id:genId("ORD"),customerName:name,status:"paid",
      createdAt:businessDate,paidAt:businessDate,items:cart,total,kasirId:user.id}]);
    ...
    <div style={{flex:1,overflowY:"auto",padding:"18px 18px 12px",display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div>
        ...
        {todayPaid.length===0?(
          ...
          <div key={o.id} style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:11,padding:"10px 13px",marginBottom:7}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
...
const Tagihan = ({orders,setOrders,menus,user,kasirs,businessDate}) => {
  ...
  const bayarSatuItem=()=>{
    ...
    setOrders(p=>p.map(o=>{
      ...
      return {...o,items,total:sisaTotal,
        ...
        paidAt:semuaLunas?businessDate:null,
  ...
  const konfirmasiLunas=()=>{
    ...
    setOrders(p=>p.map(o=>o.id===sel?{...o,status:"paid",paidAt:businessDate,kasirId:user.id,
