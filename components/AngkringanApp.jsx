...
const printOrderStrukRiwayat = async (order, kasirs, receiptSettings) => {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Struk - ${escapeHtml(order.customerName)}</title>
<style>
  @page{size:58mm auto;margin:0}
  *{box-sizing:border-box}
  html,body{margin:0;padding:0;background:#fff}
  body{font-family:'Courier New',monospace;font-size:11px;line-height:1.35;width:58mm;max-width:58mm;padding:3mm;color:#111}
  .c{text-align:center}.b{font-weight:700}.big{font-size:13px}
  .header-main{font-size:15px;letter-spacing:0.6px}
  .header-sub{font-size:10px;color:#444;margin-top:1px}
  .meta{display:grid;grid-template-columns:18mm 1fr;gap:1mm 2mm;margin:2px 0}
  .meta .label{color:#666}
  .meta .value{text-align:right;font-weight:600;word-break:break-word;overflow-wrap:anywhere}
  .dl{border-top:1px dashed #666;margin:6px 0}
  .item{margin-bottom:6px}
  .item-name{font-weight:700;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere}
  .item-note{padding-left:8px;margin-top:1px;font-size:9px;color:#666;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere}
  .item-row,.total-row{display:flex;justify-content:space-between;align-items:flex-start;gap:8px}
  .item-row .left,.total-row .left{min-width:0;color:#666;word-break:break-word;overflow-wrap:anywhere}
  .item-row .right,.total-row .right{text-align:right;font-weight:700;white-space:nowrap}
  .footer-line{font-size:10px;color:#666;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere}
  .paid-badge{display:inline-block;border:1px solid #4CAF7D;color:#4CAF7D;padding:2px 8px;border-radius:4px;font-size:10px;margin:4px 0}
</style>
</head><body>
${headerLines.map((line,idx)=>`<div class="c ${idx===0?"b header-main":"header-sub"}">${escapeHtml(line)}</div>`).join("")}
<div class="dl"></div>
<div class="meta"><div class="label">Pelanggan</div><div class="value">${escapeHtml(order.customerName || "Pelanggan")}</div></div>
<div class="meta"><div class="label">Kasir</div><div class="value">${escapeHtml(kasir?.name||"-")}</div></div>
<div class="meta"><div class="label">Waktu</div><div class="value">${escapeHtml(waktu)}</div></div>
<div class="c"><span class="paid-badge">&#10003; LUNAS</span></div>
<div class="dl"></div>
${(order.items||[]).map(item=>`<div class="item">
  <div class="item-name">${escapeHtml(item?.name || "Item")}</div>
  ${item?.note?`<div class="item-note">Catatan: ${escapeHtml(item.note)}</div>`:""}
  <div class="item-row"><span class="left">${Number(item?.qty)||0} &times; ${escapeHtml(rupiah(Number(item?.price)||0))}</span><span class="right">${escapeHtml(rupiah((Number(item?.price)||0)*(Number(item?.qty)||0)))}</span></div>
</div>`).join("")}
<div class="dl"></div>
<div class="total-row big b"><span class="left">TOTAL</span><span class="right">${escapeHtml(rupiah(order.total))}</span></div>
<div class="dl"></div>
${footerLines.map(line=>`<div class="c footer-line">${escapeHtml(line)}</div>`).join("")}
</body></html>`;
...
<div class="dl"></div>
<div class="total-row big b"><span class="left">TOTAL</span><span class="right">${escapeHtml(rupiah(order.total))}</span></div>
${mode!=="nanti"?`<div class="total-row"><span class="left">Dibayar</span><span class="right">${escapeHtml(rupiah((Number(order.total)||0)+(Number(kembalian)||0)))}</span></div>
<div class="total-row b" style="font-size:13px;margin-top:3px"><span class="left">Kembalian</span><span class="right">${escapeHtml(rupiah(kembalian||0))}</span></div>`:"<div class=\"c footer-line\" style=\"margin-top:6px\">Mohon lunasi tagihan Anda</div>"}
<div class="dl"></div>
${footerLines.map(line=>`<div class="c footer-line">${escapeHtml(line)}</div>`).join("")}
</body></html>`;
  const w=window.open("","_blank","width=380,height=760");
...
// ── POS ──
const POS = ({menus,orders,setOrders,user,businessDate,currentSessionId,kasirs,setScreen,posStep:step,setPosStep:setStep,posName:name,setPosName:setName,posCart:cart,setPosCart:setCart,receiptSettings}) => {
  ...
  if(step==="menu")return(<div className="pos-menu-screen" style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
    ...
    {/* confirm bar handled globally in App */}
...
const Tagihan = ({orders,setOrders,menus,user,kasirs,businessDate,currentSessionId,receiptSettings}) => {
  ...
  if(sel&&ord)return(<div className="tagihan-detail-screen" style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    {sheet&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:320,display:"flex",alignItems:"flex-end"}} onClick={()=>setSheet(null)}>
      <div className="fu" style={{background:"var(--bg2)",borderRadius:"20px 20px 0 0",padding:"22px 20px 28px",width:"100%",display:"flex",flexDirection:"column",gap:14,maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        <div>
...
const MenuMgmt = ({menus,setMenus,mitras,onClose}) => {
  ...
  const open=(m=null)=>{if(m){setEid(m.id);setForm({name:m.name,price:String(m.price),category:m.category,available:m.available,mitraId:m.mitraId||null,hargaMitra:m.hargaMitra?String(m.hargaMitra):"",suhu:m.suhu||"Tidak Ada"});}
    else{setEid(null);setForm({name:"",price:"",category:getCategoryOptions(menus,false)[0]||"Kopi",available:true,mitraId:null,hargaMitra:"",suhu:"Tidak Ada"});}setShow(true);};
    ...
    {show&&(<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.75)",zIndex:200,display:"flex",alignItems:"flex-end"}} onClick={()=>setShow(false)}>
      <div className="fu" style={{background:"var(--bg2)",borderRadius:"20px 20px 0 0",padding:"20px 20px 34px",width:"100%",display:"flex",flexDirection:"column",gap:13,maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div><p style={{fontSize:11,color:"var(--muted)",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:8}}>Kategori</p>
          <div style={{display:"flex",gap:8}}>
            {getCategoryOptions(menus,false).map(c=>(<button key={c} onClick={()=>setForm(p=>({...p,category:c}))} style={{flex:1,padding:"9px",borderRadius:9,
      ...
      {!overlay&&(<div className="app-frame">
        ...
        <div key={screen} className="screen-shell fi">
          ...
          {screen==="pos"&&(user.role==="owner"||sessionOpen?
            <POS menus={menus} orders={orders} setOrders={setOrders} user={user} businessDate={businessDate} currentSessionId={currentSessionId} kasirs={kasirs} setScreen={setScreen} posStep={posStep} setPosStep={setPosStep} posName={posName} setPosName={setPosName} posCart={posCart} setPosCart={setPosCart} receiptSettings={receiptSettings}/>
        ...
        {/* ── Global Floating Konfirmasi Bar (muncul di semua layar selama ada item di keranjang) ── */}
        {posCart.length>0&&posStep==="menu"&&screen!=="pos"&&(
          <div
          ...
          >
            <div style={{background:"rgba(0,0,0,0.18)",borderRadius:8,padding:"4px 12px"}}>
              <span style={{color:"#fff",fontWeight:700,fontSize:13}}>{posCart.reduce((s,c)=>s+c.qty,0)} item</span>
            </div>
            <span style={{color:"#fff",fontWeight:700,fontSize:14}}>Konfirmasi →</span>
            <span className="sora" style={{color:"#fff",fontWeight:800,fontSize:14}}>{rupiah(posCart.reduce((s,c)=>s+(c.price*c.qty),0))}</span>
          </div>
        )}
      </div>)}
    </div>
