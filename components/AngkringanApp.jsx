...
export default function AngkringanApp() {
  ...
  // Daftarkan toast & close-overlay bridge
  useEffect(()=>{
    window.__angkringanToast = (msg)=>setBackToast(msg);
    window.__angkringanCloseOverlay = ()=>{
      if(overlay){setOverlay(null);return true;}
      if(navOpen){setNavOpen(false);return true;}
      return false;
    };
    return ()=>{
      delete window.__angkringanToast;
      delete window.__angkringanCloseOverlay;
    };
  },[overlay,navOpen]);

  // ── Tombol BACK HP: tekan 1x → Dashboard, tekan 2–3x dalam 2 detik → keluar aplikasi ──
  const backPressCount = useRef(0);
  const backPressTimer = useRef(null);
  useEffect(()=>{
    // Pastikan selalu ada state di history agar popstate bisa ditangkap
    const pushDummy = () => {
      if(window.history.state?.angkringan !== true){
        window.history.pushState({angkringan:true},"",window.location.href);
      }
    };
    pushDummy();

    const handlePopState = (e) => {
      // Selalu push ulang agar back selanjutnya masih bisa ditangkap
      pushDummy();

      // Jika overlay terbuka, tutup overlay dulu
      if(window.__angkringanCloseOverlay) {
        window.__angkringanCloseOverlay();
        backPressCount.current = 0;
        return;
      }

      // Jika sudah di home, hitung berapa kali back
      if(screen==="home"){
        backPressCount.current += 1;
        clearTimeout(backPressTimer.current);
        if(backPressCount.current >= 2){
          // 2x back di dashboard → keluar
          backPressCount.current = 0;
          if(window.__angkringanToast) window.__angkringanToast("");
          if(typeof window?.Capacitor !== "undefined"){
            try { window.Capacitor.exitApp(); } catch{}
          } else {
            window.history.go(-2);
          }
          return;
        }
        // Tampilkan toast hint
        if(window.__angkringanToast) window.__angkringanToast("Tekan sekali lagi untuk keluar");
      ...
      // Jika tidak di home → kembali ke dashboard
      backPressCount.current = 0;
      setScreen("home");
    };
