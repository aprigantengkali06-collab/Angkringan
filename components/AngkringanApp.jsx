...
const POS = ({menus,orders,setOrders,user,businessDate,kasirs,setScreen}) => {
  ...
  if(step==="name")return(
    <div style={{flex:1,overflowY:"auto",padding:"18px 18px 12px",display:"flex",flexDirection:"column",gap:16}}>
      ...
      {/* Tagihan belum bayar hari ini */}
      {todayOpen.length>0&&(
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
            <p style={{fontSize:11,color:"var(--red)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em"}}>
              ⏳ Belum Bayar ({todayOpen.length})
            </p>
            <button onClick={()=>setScreen("tagihan")} style={{fontSize:12,color:"var(--amber)",fontWeight:600,background:"none",border:"none",cursor:"pointer"}}>
              Kelola →
            </button>
          </div>
          {todayOpen.map(o=>(
      ...
      {/* Histori transaksi lunas hari ini */}
      <div>
        <p style={{fontSize:11,color:"var(--muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:9}}>
          ✅ Transaksi Hari Ini ({todayPaid.length})
        </p>
        {todayPaid.length===0?(
          <Card style={{textAlign:"center",padding:16}}>
            <p style={{color:"var(--muted)",fontSize:13}}>Belum ada transaksi hari ini</p>
          </Card>
        ):todayPaid.map((o,i)=>(
          <div key={o.id} style={{background:"var(--card)",border:"1px solid var(--border)",
            borderRadius:11,padding:"10px 13px",marginBottom:7}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div style={{display:"flex",alignItems:"center",gap:7}}>
                <span style={{width:22,height:22,borderRadius:6,background:"var(--green-dim)",
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"var(--green)",flexShrink:0}}>{i+1}</span>
                <p style={{color:"var(--text)",fontWeight:600,fontSize:14}}>{o.customerName}</p>
              </div>
...
const MenuMgmt = ({menus,setMenus,mitras,onClose}) => {
  ...
  const [form,setForm]=useState({name:"",price:"",category:"Kopi",available:true,mitraId:null,hargaMitra:""});
  const [cat,setCat]=useState("Semua");
  const open=(m=null)=>{if(m){setEid(m.id);setForm({name:m.name,price:String(m.price),category:m.category,available:m.available,mitraId:m.mitraId||null,hargaMitra:m.hargaMitra?String(m.hargaMitra):""});}
    else{setEid(null);setForm({name:"",price:"",category:"Kopi",available:true,mitraId:null,hargaMitra:""});}setShow(true);};
  const save=()=>{if(!form.name||!form.price)return;
    const menuData={...form,price:parseInt(form.price),mitraId:form.mitraId||null,hargaMitra:form.mitraId&&form.hargaMitra?parseInt(form.hargaMitra):null};
...
export default function AngkringanApp() {
  ...
  useEffect(()=>{
    ...
    supabase.from("menus").select("*").then(({data})=>{ if(data?.length) setMenus(data.map(r=>({id:r.id,name:r.name,price:r.price,category:r.category,available:r.available,mitraId:r.mitra_id||null,hargaMitra:r.harga_mitra||null}))); });
  ...
  // Sync menus
  useEffect(()=>{
    menus.forEach(m=>supabase.from("menus").upsert({id:m.id,name:m.name,price:m.price,category:m.category,available:m.available,mitra_id:m.mitraId||null,harga_mitra:m.hargaMitra||null}).then());
  },[menus]);
