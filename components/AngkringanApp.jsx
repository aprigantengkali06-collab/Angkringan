  ...
  <style>{`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @media (min-width: 560px) and (orientation: landscape), (min-width: 560px) and (max-height: 500px){
      ...
      .hdr-shell{padding:10px 16px 8px !important}
    ...
    @media (min-width: 1024px) and (orientation: landscape), (min-width: 1024px) and (max-height: 500px){
      ...
      .hdr-shell{padding:12px 18px 10px !important}
...
const Hdr = ({title,sub,left,right}) => (
  <div className="hdr-shell glass-card" style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,
    padding:"10px 16px 8px",borderBottom:"1px solid rgba(215,226,240,0.92)",flexShrink:0,
...
const Dashboard = ({orders,expenses,setExpenses,user,setScreen,target,setTarget,kasirs,mitras,menus,businessDate,sessionOpen,sessionDate,onBuka,onTutup}) => {
  ...
  const totalMitraModal = mitraStats.reduce((s,m)=>s+m.modal,0);
      ...
      <div className="dashboard-summary-grid" style={{gap:9,alignItems:"start"}}>
        ...
        {mitraStats.length>0&&(
          <div className="fu s3" style={{background:"var(--card)",border:"1px solid rgba(124,58,237,0.2)",borderRadius:13,padding:"12px 14px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:8}}>
              ...
              <span style={{color:"var(--green)",fontSize:11,fontWeight:700,background:"var(--green-dim)",padding:"4px 8px",borderRadius:999}}>Profit total {rupiah(totalMitraProfit)}</span>
            </div>
            {mitraStats.map((m,i)=>{
        ...
        {(()=>{const s={val:bersih,color:bersih>=0?"var(--amber)":"var(--red)",bg:bersih>=0?"var(--amber-dim)":"var(--red-dim)"};return(
          <div className={mitraStats.length>0?"fu s4":"fu s3"} style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:13,
            padding:"12px 14px",display:"flex",flexDirection:"column",alignItems:"stretch",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:11}}>
              <div style={{width:36,height:36,borderRadius:10,background:s.bg,display:"flex",alignItems:"center",justifyContent:"center"}}>
                <div style={{width:9,height:9,borderRadius:"50%",background:s.color}}/>
              </div>
            ...
            <div style={{paddingLeft:2}}>
              ...
              <p style={{color:"var(--muted)",fontSize:10,marginTop:3}}>Ringkasan kas dan kontribusi mitra</p>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:7}}>
              {[
                {label:"Pemasukan",val:rupiah(pemasukan),color:"var(--green)",bg:"var(--green-dim)"},
                {label:"Pengeluaran",val:rupiah(pengeluaran),color:"var(--red)",bg:"var(--red-dim)"},
                {label:"Modal Mitra",val:rupiah(totalMitraModal),color:"var(--purple)",bg:"var(--purple-dim)"},
                {label:"Profit Mitra",val:rupiah(totalMitraProfit),color:"var(--blue)",bg:"var(--blue-dim)"},
              ].map(item=>(
                <div key={item.label} style={{background:item.bg,borderRadius:10,padding:"7px 8px"}}>
                  <p style={{color:"var(--muted)",fontSize:9,marginBottom:2}}>{item.label}</p>
                  <p className="sora" style={{color:item.color,fontWeight:700,fontSize:11}}>{item.val}</p>
                </div>
              ))}
            </div>
          </div>
