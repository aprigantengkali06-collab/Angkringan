...
const POS = ({menus,orders,setOrders,user,businessDate,kasirs,setScreen}) => {
  ...
  const submit=now=>{setOrders(p=>[...p,{id:genId("ORD"),customerName:name,status:now?"paid":"open",
    createdAt:businessDate,paidAt:now?businessDate:null,items:cart,total,kasirId:user.id}]);reset();};
...
const Tagihan = ({orders,setOrders,menus,user,kasirs,businessDate}) => {
  ...
  const lunas=()=>{setOrders(p=>p.map(o=>o.id===sel?{...o,status:"paid",paidAt:businessDate,kasirId:user.id}:o));setSel(null);};
...
const MenuMgmt = ({menus,setMenus,mitras,onClose}) => {
  ...
  const save=()=>{if(!form.name||!form.price)return;
    ...
    if(eid)setMenus(p=>p.map(m=>m.id===eid?{...m,...menuData}:m));
    else setMenus(p=>[...p,{id:Date.now(),...menuData}]);setShow(false);};
