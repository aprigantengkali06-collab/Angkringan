...
const Tagihan = ({orders,setOrders,menus,user,kasirs,businessDate}) => {
  ...
  const [bayarItem,setBayarItem]=useState(null); // {menuId, name, price, qty, subtotal, lineKey}
  const [uangItem,setUangItem]=useState("");
  const getLineKey = (item, idx=0) => item?.cartKey || `${item?.menuId||"menu"}::${item?.name||""}::${item?.note||""}::${item?.suhu||""}::${idx}`;
  const getItemsTotal = items => items.reduce((s,i)=>s+(Number(i.price)||0)*(Number(i.qty)||0),0);
  const openOrders=orders.filter(o=>o.status==="open"&&o.total>0);
  ...
  // Bayar satu item → langsung pecah jadi transaksi paid supaya pemasukan masuk saat itu juga
  const bayarSatuItem=()=>{
    if(!bayarItem||!uangItem||parseInt(uangItem)<bayarItem.subtotal)return;

    const currentOrder = orders.find(o=>o.id===sel);
    const remainingCount = currentOrder
      ? currentOrder.items.filter((item,idx)=>getLineKey(item,idx)!==bayarItem.lineKey && !item.paid).length
      : 0;
    ...
    setOrders(prev=>prev.flatMap(o=>{
      ...
      const paidLine = o.items.find((item,idx)=>getLineKey(item,idx)===bayarItem.lineKey);
      if(!paidLine) return [o];

      const remainingItems = o.items.filter((item,idx)=>getLineKey(item,idx)!==bayarItem.lineKey && !item.paid)
        .map(({paid,...rest})=>rest);
      const paidItem = {...paidLine, paid:true};
      ...
      return [
        ...
        },
        {
          ...
          status:"paid",
          createdAt:o.createdAt || businessDate,
          paidAt,
          items:[paidItem],
          total:paidTotal,
          kasirId:user.id,
    ...
    {adding?(
      ...
      <div style={{flex:1,overflowY:"auto",padding:"17px",display:"flex",flexDirection:"column",gap:13}}>
        <Card>{ord.items.map((item,i)=>{
          ...
          const lineKey=getLineKey(item,i);
