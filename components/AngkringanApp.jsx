...
// paidAt bisa berupa "YYYY-MM-DD" (lama) atau ISO string penuh
const paidAtDate = pa => pa ? pa.slice(0,10) : null;
const fmtWaktu = pa => {
  if(!pa) return "";
...
const todayStr = fmt(today); // fallback untuk data statis
const rupiah = n => "Rp " + Number(n).toLocaleString("id-ID");
const genId = p => p + Date.now().toString().slice(-5);
// Timestamp lokal (bukan UTC) — supaya slice(0,10) = tanggal WIB yang benar
const localISO = () => {
  const d = new Date();
...
const printDayPDF = (date, orders, expenses, kasirs) => {
  const paid = orders.filter(o=>o.status==="paid"&&paidAtDate(o.paidAt)===date);
...
const printMonthPDF = (monthKey, monthLabel, orders, expenses, kasirs) => {
  const paid = orders.filter(o=>o.status==="paid"&&paidAtDate(o.paidAt)?.startsWith(monthKey));
...
// Level 3: Detail Hari
const DayDetail = ({date, orders, expenses, kasirs, onBack, businessDate}) => {
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        ...
        <button onClick={()=>printDayPDF(date,[...orders,...(addExpenses.length?[]:[])]
          .filter(o=>o.status==="paid"&&paidAtDate(o.paidAt)===date).length>=0?orders:[...orders],
      ...
      <div>
          ...
          {paid.length===0?(
            ...
            const sorted=[...paid].sort((a,b)=>(b.paidAt||"").localeCompare(a.paidAt||""));
            const PAGE=10;
            ...
            return(<>
              {slice.map((o,idx)=>{
                const tanggalWaktu=fmtTanggalWaktu(o.paidAt);
                return(
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        ...
                        <div>
                          ...
                          {tanggalWaktu&&<p style={{color:"var(--muted)",fontSize:10,marginTop:1}}>🕐 {tanggalWaktu}</p>}
                        </div>
                      </div>
...
// ── Keuangan Main ──
const Keuangan = ({orders, expenses, setExpenses, kasirs, businessDate}) => {
  ...
  const getMonthSummary = (monthKey) => {
    const pemasukan = orders.filter(o=>o.status==="paid"&&paidAtDate(o.paidAt)?.startsWith(monthKey)).reduce((s,o)=>s+o.total,0);
  ...
  const getDaySummary = (ds) => {
    const pemasukan = orders.filter(o=>o.status==="paid"&&paidAtDate(o.paidAt)===ds).reduce((s,o)=>s+o.total,0);
        ...
        {selDay?(
          <div className="fi">
            ...
            <DayDetail date={selDay} orders={orders} expenses={expenses}
              kasirs={kasirs} onBack={()=>setSelDay(null)} businessDate={businessDate}/>
          </div>
...
// ── Dashboard ──
const Dashboard = ({orders,expenses,setExpenses,user,setScreen,target,setTarget,kasirs,businessDate,sessionOpen,sessionDate,onBuka,onTutup}) => {
  ...
  const paidToday=orders.filter(o=>o.status==="paid"&&paidAtDate(o.paidAt)===businessDate);
  const openOrders=orders.filter(o=>o.status==="open");
  const expsToday=expenses.filter(e=>e.date===businessDate);
  const pemasukan=paidToday.reduce((s,o)=>s+o.total,0);
  const pengeluaran=expsToday.reduce((s,e)=>s+e.amount,0);
  ...
  const chartData=useMemo(()=>Array.from({length:7},(_,i)=>{
    ...
    return {day:d.toLocaleDateString("id-ID",{weekday:"short"}),
      total:orders.filter(o=>o.status==="paid"&&paidAtDate(o.paidAt)===ds).reduce((s,o)=>s+o.total,0),isToday:ds===businessDate};
  }),[orders,businessDate]);

  const saveExp=()=>{
    if(!expDesc||!expAmt)return;
    const newExp={id:genId("EXP"),description:expDesc,amount:parseInt(expAmt),date:businessDate};
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:"16px 18px 12px"}}>
      ...
      {/* Status untuk kasir */}
      {user.role==="kasir"&&(
        <div className="fu" style={{background:sessionOpen?"var(--green-dim)":"rgba(239,68,68,0.07)",
          border:`1px solid ${sessionOpen?"rgba(16,185,129,0.3)":"rgba(239,68,68,0.3)"}`,
          borderRadius:13,padding:"11px 15px",marginBottom:12,display:"flex",alignItems:"center",gap:9}}>
          <div style={{width:9,height:9,borderRadius:"50%",background:sessionOpen?"var(--green)":"var(--red)",
            boxShadow:sessionOpen?"0 0 8px var(--green)":"none",flexShrink:0}}/>
          <div>
            <p style={{color:sessionOpen?"var(--green)":"var(--red)",fontWeight:700,fontSize:13}}>
              {sessionOpen?"Sesi Sedang Berjalan":"Angkringan Belum Buka"}
            </p>
            <p style={{color:"var(--muted)",fontSize:11,marginTop:1}}>
              {sessionOpen?`Rekap tanggal: ${sessionDate}`:"Tunggu owner membuka sesi"}
            </p>
          </div>
        </div>
      )}
      ...
      <div style={{marginTop:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
          <p style={{fontSize:11,color:"var(--muted)",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em"}}>
            🧾 Pengeluaran Hari Ini ({expsToday.length})
          </p>
...
// ── POS ──
const POS = ({menus,orders,setOrders,user,businessDate,kasirs,setScreen}) => {
  ...
  const cartKey=(menuId,suhu)=>`${menuId}_${suhu}`;
  const qtyOf=(menuId,suhu)=>cart.find(c=>c.cartKey===cartKey(menuId,suhu))?.qty||0;
  ...
  const addFromSheet=()=>{
    ...
    const key=needSuhu?cartKey(m.id,suhu):`${m.id}_`;
    const displayName=needSuhu?`${m.name} (${suhu})`:m.name;
    setCart(p=>{
      const e=p.find(c=>c.cartKey===key);
      if(e)return p.map(c=>c.cartKey===key?{...c,qty:c.qty+1,note:sheetNote||c.note}:c);
      return [...p,{cartKey:key,menuId:m.id,name:displayName,price:m.price,qty:1,suhu,note:sheetNote}];
    });
    setSheet(null);setSheetNote("");
  };

  const chg=(key,d)=>setCart(p=>p.map(c=>c.cartKey===key?{...c,qty:c.qty+d}:c).filter(c=>c.qty>0));
  const reset=()=>{setStep("name");setName("");setCart([]);setCat("Semua");setSearch("");};
  ...
  const submit=now=>{
    ...
    setOrders(p=>[...p,{id:genId("ORD"),customerName:name,status:"open",
      createdAt:businessDate,paidAt:null,items:cart,total,kasirId:user.id}]);reset();
  };
  const konfirmasiBayar=()=>{
    if(!uangDibayar||parseInt(uangDibayar)<total)return;
    setOrders(p=>[...p,{id:genId("ORD"),customerName:name,status:"paid",
      createdAt:businessDate,paidAt:localISO(),items:cart,total,kasirId:user.id}]);
    setBayarModal(false);setUangDibayar("");reset();
  };

  // Histori transaksi hari ini
  const todayOrders=orders.filter(o=>o.createdAt===businessDate).sort((a,b)=>b.id.localeCompare(a.id));
  const todayPaid=todayOrders.filter(o=>o.status==="paid");
  const todayOpen=todayOrders.filter(o=>o.status==="open");

  if(step==="name")return(
    <div style={{flex:1,overflowY:"auto",padding:"18px 18px 12px",display:"flex",flexDirection:"column",gap:16}}>
      <div>
      ...
      <div>
        <p style={{fontSize:11,color:"var(--muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:9}}>✅ Transaksi Hari Ini ({todayPaid.length})</p>
        {todayPaid.length===0?(
          <Card style={{textAlign:"center",padding:16}}><p style={{color:"var(--muted)",fontSize:13}}>Belum ada transaksi hari ini</p></Card>
        ):(()=>{
          const sorted=[...todayPaid].sort((a,b)=>(b.paidAt||"").localeCompare(a.paidAt||""));
          const PAGE=10;
          const total_pages=Math.ceil(sorted.length/PAGE);
          ...
          return(<>
            {slice.map((o,i)=>{
              const tanggalWaktu=fmtTanggalWaktu(o.paidAt);
              return(<div key={o.id} style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:11,padding:"10px 13px",marginBottom:7}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
  ...
  if(step==="menu")return(<div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
    <div style={{padding:"11px 18px 9px",borderBottom:"1px solid var(--border)",flexShrink:0}}>
      ...
      {/* Search bar */}
      <div style={{display:"flex",alignItems:"center",gap:8,background:"var(--card2)",border:"1px solid var(--border)",
        borderRadius:11,padding:"8px 12px",marginBottom:9}}>
        ...
        {search&&<button onClick={()=>setSearch("")} style={{color:"var(--muted)",fontSize:16,lineHeight:1}}>×</button>}
      </div>
      {!search&&quickMenus.length>0&&(<div style={{marginBottom:9}}>
      ...
      {!search&&<CatBar val={cat} set={setCat}/>}
    </div>
    <div style={{flex:1,overflowY:"auto",padding:"11px 18px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
      {filtered.length===0&&(
        <div style={{gridColumn:"1/-1",textAlign:"center",padding:32}}>
          <p style={{color:"var(--muted)",fontSize:13}}>Menu "{search}" tidak ditemukan</p>
        </div>
      )}
      {filtered.map(m=>{
        ...
        return(<div key={m.id} style={{background:q>0?"rgba(245,166,35,0.07)":"var(--card)",
          ...
          <p style={{color:"var(--text)",fontWeight:600,fontSize:13,lineHeight:1.3,marginBottom:4}}>{m.name}</p>
          {hasSuhu&&(
          ...
          <button onClick={()=>openSheet(m)} style={{width:"100%",padding:"7px",borderRadius:8,
            ...
            color:q>0?"#fff":"var(--amber)",border:"1px solid rgba(245,166,35,0.2)",fontSize:12,fontWeight:600}}>
            {q>0?`${q}× Tambah`:"+ Tambah"}
          </button>
        </div>);
      })}
...
// ── Tagihan ──
const Tagihan = ({orders,setOrders,menus,user,kasirs,businessDate}) => {
  const [sel,setSel]=useState(null);
  const [adding,setAdding]=useState(false);
  const [cat,setCat]=useState("Semua");
  // per-item bayar modal
  const [bayarItem,setBayarItem]=useState(null); // {menuId, name, price, qty, subtotal}
  ...
  const addItem=m=>setOrders(p=>p.map(o=>{
    ...
    return {...o,items,total:items.filter(i=>!i.paid).reduce((s,i)=>s+i.price*i.qty,0)};
  }));

  // Bayar satu item
  const bayarSatuItem=()=>{
    if(!bayarItem||!uangItem||parseInt(uangItem)<bayarItem.subtotal)return;
    setOrders(p=>p.map(o=>{
      if(o.id!==sel)return o;
      const items=o.items.map(i=>i.menuId===bayarItem.menuId?{...i,paid:true}:i);
      const sisaTotal=items.filter(i=>!i.paid).reduce((s,i)=>s+i.price*i.qty,0);
      const totalAsli=items.reduce((s,i)=>s+i.price*i.qty,0);
      const semuaLunas=items.every(i=>i.paid);
      return {...o,items,total:semuaLunas?totalAsli:sisaTotal,
        status:semuaLunas?"paid":"open",
        paidAt:semuaLunas?localISO():null,
        kasirId:semuaLunas?user.id:o.kasirId};
    }));
    setBayarItem(null);setUangItem("");
    // kalau semua item sudah paid, kembali ke list
    const updOrd=orders.find(o=>o.id===sel);
    if(updOrd){
      const remaining=updOrd.items.filter(i=>!i.paid&&i.menuId!==bayarItem.menuId);
      if(remaining.length===0)setSel(null);
    }
  };

  // Lunasi semua sekaligus (full bayar)
  ...
  const konfirmasiLunas=()=>{
    ...
    setOrders(p=>p.map(o=>o.id===sel?{...o,status:"paid",paidAt:localISO(),kasirId:user.id,
      items:o.items.map(i=>({...i,paid:true}))}:o));
  ...
  if(sel&&ord)return(<div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    <Hdr title={ord.customerName} sub={`Sisa ${rupiah(ord.total)}`}
    ...
    {adding?(
      ...
      <div style={{flex:1,overflowY:"auto",padding:"17px",display:"flex",flexDirection:"column",gap:13}}>
        <Card>{ord.items.map((item,i)=>{
          ...
          return(<div key={item.menuId} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0,marginLeft:8}}>
              ...
              {!item.paid&&(
                <button onClick={()=>{setBayarItem({...item,subtotal});setUangItem("");}}
  ...
  return(<div style={{flex:1,overflowY:"auto",padding:"17px"}}>
    <div className="fu" style={{marginBottom:12}}>
      ...
      <p style={{color:"var(--muted)",fontSize:13,marginTop:3}}>{openOrders.length} pelanggan · <span style={{color:"var(--amber)",fontWeight:700}}>{rupiah(openOrders.reduce((s,o)=>s+o.total,0))}</span> potensi masuk</p>
    </div>
    {openOrders.length===0?(<Card style={{textAlign:"center",padding:32}}><p style={{fontSize:28,marginBottom:8}}>🎉</p><p style={{color:"var(--muted)"}}>Semua tagihan lunas!</p></Card>)
    :openOrders.map((o,i)=>(<div key={o.id} onClick={()=>setSel(o.id)} style={{background:"var(--card)",border:"1px solid var(--border)",
      ...
      <div style={{flex:1}}>
        ...
        <p style={{color:"var(--muted)",fontSize:12,marginTop:2}}>{o.items.map(i=>`${i.name} ×${i.qty}${i.paid?" ✓":""}`).join(" • ")}</p>
...
// ── Tim ──
const Tim = ({kasirs,setKasirs,mitras,setMitras,onClose}) => {
  ...
  return(<div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    <Hdr title="Manajemen Tim" sub="Kasir & Mitra" right={
    ...
    {/* Tab bar */}
    <div style={{display:"flex",padding:"10px 18px 0",gap:8,borderBottom:"1px solid var(--border)",flexShrink:0}}>
      {[{k:"kasir",label:`Kasir (${kasirs.length})`},{k:"mitra",label:`Mitra (${mitras.length})`}].map(t=>(
        <button key={t.k} onClick={()=>setTab(t.k)} style={{padding:"8px 16px",borderRadius:"10px 10px 0 0",fontWeight:700,fontSize:13,
          ...
          borderBottom:tab===t.k?"1px solid var(--card)":"none",marginBottom:tab===t.k?-1:0}}>
          {t.label}
        </button>
      ))}
    </div>
      ...
      {tab==="kasir"?(<>
        {kasirs.map((k,i)=>(<Card key={k.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><p style={{color:"var(--text)",fontWeight:700}}>{k.name}</p></div>
          </div>
...
export default function AngkringanApp() {
  ...
  const [sessionDate, setSessionDate] = useState(()=>{try{const s=localStorage.getItem("sessionDate");return s?JSON.parse(s):fmt(new Date());}catch{return fmt(new Date());}});
  const businessDate = sessionDate || fmt(new Date());
  ...
  // Flag: apakah data Supabase sudah selesai di-load?
  // Sync effect TIDAK boleh jalan sebelum ini true, supaya tidak overwrite data Supabase
  // dengan data lama dari localStorage/MENUS0 saat pertama load.
  const initialized = useRef(false);

  useEffect(()=>{if(user)localStorage.setItem("user",JSON.stringify(user));else localStorage.removeItem("user");},[user]);
  useEffect(()=>{localStorage.setItem("target",JSON.stringify(target));supabase.from("settings").upsert({key:"target",value:target}).then();},[target]);
  useEffect(()=>{localStorage.setItem("sessionOpen",JSON.stringify(sessionOpen));supabase.from("settings").upsert({key:"session_open",value:String(sessionOpen)}).then();},[sessionOpen]);
  useEffect(()=>{localStorage.setItem("sessionDate",JSON.stringify(sessionDate));supabase.from("settings").upsert({key:"session_date",value:sessionDate}).then();},[sessionDate]);

  // Load data dari Supabase saat pertama mount.
  // Setelah semua fetch selesai, baru set initialized=true supaya sync effects boleh jalan.
  useEffect(()=>{
    Promise.all([
      supabase.from("kasirs").select("*").then(({data})=>{ if(data?.length) setKasirs(data.map(r=>({id:r.id,name:r.name,password:r.password}))); }),
      supabase.from("mitras").select("*").then(({data})=>{ if(data?.length) setMitras(data.map(r=>({id:r.id,name:r.name,pemilik:r.pemilik}))); }),
      supabase.from("menus").select("*").then(({data})=>{ if(data?.length) setMenus(data.map(r=>({id:r.id,name:r.name,price:r.price,category:r.category,available:r.available,mitraId:r.mitra_id||null,hargaMitra:r.harga_mitra||null,suhu:r.suhu||null}))); }),
      ...
      supabase.from("settings").select("*").then(({data})=>{
        if(data) {
          const t=data.find(r=>r.key==="target"); if(t) setTarget(Number(t.value));
          const so=data.find(r=>r.key==="session_open"); if(so) setSessionOpen(so.value==="true");
          const sd=data.find(r=>r.key==="session_date"); if(sd) setSessionDate(sd.value);
        }
      }),
    ]).finally(()=>{ initialized.current=true; });
  },[]);

  // Sync kasirs — hanya jalan setelah data Supabase di-load, dan simpan ke localStorage
  useEffect(()=>{
  ...
  // Sync mitras — hanya jalan setelah data Supabase di-load, dan simpan ke localStorage
  useEffect(()=>{
    if(!initialized.current) return;
  ...
  // Sync menus — hanya jalan setelah data Supabase di-load, dan simpan ke localStorage
  useEffect(()=>{
    if(!initialized.current) return;
  ...
  // Sync orders — hanya jalan setelah data Supabase di-load, dan simpan ke localStorage
  useEffect(()=>{
    if(!initialized.current) return;
  ...
  // Sync expenses — hanya jalan setelah data Supabase di-load, dan simpan ke localStorage
  useEffect(()=>{
    if(!initialized.current) return;
  ...
  return(<><FontStyle/>
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"var(--bg)",maxWidth:480,margin:"0 auto",position:"relative",overflow:"hidden"}}>
      ...
      {overlay==="tim"&&<Tim kasirs={kasirs} setKasirs={setKasirs} mitras={mitras} setMitras={setMitras} onClose={()=>setOverlay(null)}/>}
      {!overlay&&(<>
        ...
        <div key={screen} className="fi" style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          {screen==="home"&&<Dashboard orders={orders} expenses={expenses} setExpenses={setExpenses} user={user} setScreen={setScreen} target={target} setTarget={setTarget} kasirs={kasirs} businessDate={businessDate} sessionOpen={sessionOpen} sessionDate={sessionDate} onBuka={handleBuka} onTutup={handleTutup}/>}
          {screen==="pos"&&<POS menus={menus} orders={orders} setOrders={setOrders} user={user} businessDate={businessDate} kasirs={kasirs} setScreen={setScreen}/>}
          {screen==="tagihan"&&<Tagihan orders={orders} setOrders={setOrders} menus={menus} user={user} kasirs={kasirs} businessDate={businessDate}/>}
          {screen==="keuangan"&&user.role==="owner"&&<Keuangan orders={orders} expenses={expenses} setExpenses={setExpenses} kasirs={kasirs} businessDate={businessDate}/>}
        </div>
