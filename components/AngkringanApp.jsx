...
const FontStyle = () => (
  <style>{`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    html,body{background:linear-gradient(180deg,#0F172A 0%,#111827 100%)}
    ...
    .app-shell{height:100vh;display:flex;flex-direction:column;background:radial-gradient(circle at top, rgba(99,102,241,0.14), transparent 28%),linear-gradient(180deg,#F9FBFF 0%,#ECF3FF 100%);max-width:520px;margin:0 auto;position:relative;overflow:hidden}
    ...
    @media (min-width: 700px) and (orientation: landscape){
      .app-shell{max-width:min(1440px,calc(100vw - 16px));height:calc(100vh - 8px);margin:4px auto;border-radius:24px;border:1px solid rgba(148,163,184,0.28);box-shadow:0 22px 60px rgba(15,23,42,0.18)}
      .app-frame{grid-template-columns:96px minmax(0,1fr);grid-template-rows:auto minmax(0,1fr);grid-template-areas:"nav header" "nav content"}
      .nav-shell{display:flex !important;flex-direction:column !important;justify-content:flex-start;padding:16px 8px !important;gap:10px;border-top:none !important;border-right:1px solid var(--border);background:linear-gradient(180deg,rgba(15,23,42,0.97) 0%,rgba(30,41,59,0.95) 100%) !important}
      .nav-brand{display:flex !important;align-items:center;justify-content:center;padding-bottom:4px}
      .nav-btn{width:100%;min-height:60px !important;padding:12px 8px !important;border-radius:16px;justify-content:center}
      .nav-btn.active{color:#fff}
      .nav-btn.active::after{background:linear-gradient(180deg, rgba(245,158,11,0.22) 0%, rgba(37,99,235,0.24) 100%);box-shadow:0 18px 30px rgba(15,23,42,0.28)}
      .hdr-shell{padding:16px 18px 12px !important}
    ...
    @media (min-width: 1100px) and (orientation: landscape){
      .app-shell{max-width:min(1440px,calc(100vw - 36px));height:calc(100vh - 24px);margin:12px auto;border-radius:30px;box-shadow:0 28px 80px rgba(15,23,42,0.22)}
      .app-frame{grid-template-columns:112px minmax(0,1fr)}
      .nav-shell{padding:22px 10px !important}
      .nav-btn{min-height:68px !important;padding:14px 10px !important;border-radius:18px}
      .hdr-shell{padding:20px 24px 16px !important}
      .dashboard-scroll,.pos-name-screen,.pos-confirm-screen,.tagihan-list-screen{padding-left:24px !important;padding-right:24px !important}
    }
...
const SuccessOverlay = ({type,kembalian=0,onPrint,onBack,printLabel="Cetak Struk",backLabel="Kembali"}) => {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:9999,
      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:20,padding:"24px 20px"}}>
      <div style={{width:120,height:120,borderRadius:"50%",
        ...
        <svg width={62} height={62} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
...
const Hdr = ({title,sub,right}) => (
  <div className="hdr-shell glass-card" style={{display:"flex",alignItems:"center",justifyContent:"space-between",
    padding:"15px 18px 12px",borderBottom:"1px solid rgba(215,226,240,0.92)",flexShrink:0,
    boxShadow:"0 10px 35px rgba(15,23,42,0.05)"}}>
    <div>
      <h2 className="sora" style={{fontSize:18,fontWeight:800,color:"var(--text)",letterSpacing:"-0.03em"}}>{title}</h2>
      {sub&&<p style={{fontSize:11,color:"var(--muted)",marginTop:3,fontWeight:500}}>{sub}</p>}
    </div>
    {right}
  </div>
);
...
const Dashboard = ({orders,expenses,setExpenses,user,setScreen,target,setTarget,kasirs,mitras,menus,businessDate,sessionOpen,sessionDate,onBuka,onTutup}) => {
  ...
  return(
    <div className="dashboard-scroll" style={{flex:1,overflowY:"auto",padding:"16px 18px 12px"}}>
      ...
      <div className="dashboard-summary-grid" style={{gap:9}}>
        {[{label:"Pemasukan",val:pemasukan,color:"var(--green)",bg:"var(--green-dim)"},
          {label:"Pengeluaran",val:pengeluaran,color:"var(--red)",bg:"var(--red-dim)"},
        ].map((s,i)=>(
        ...
        {/* Card Mitra */}
        {mitraStats.length>0&&(
          <div className="fu s3" style={{background:"var(--card)",border:"1px solid rgba(124,58,237,0.2)",borderRadius:13,padding:"13px 16px"}}>
            ...
            {mitraStats.map((m,i)=>{
              ...
              return(
                <div key={m.id} style={{padding:"9px 10px",borderRadius:10,
    ...
    <div className="app-shell">
      ...
      {!overlay&&(<div className="app-frame">
        <Hdr {...(titles[screen]||titles.home)} right={
          user.role==="owner"?(
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <button onClick={()=>setOverlay("tim")} style={{padding:"7px 12px",borderRadius:12,background:"var(--blue-dim)",color:"var(--blue)",border:"1px solid rgba(37,99,235,0.22)",fontSize:12,fontWeight:700}}>Tim</button>
              <button onClick={()=>setOverlay("menu")} style={{padding:"7px 12px",borderRadius:12,background:"var(--amber-dim)",color:"var(--amber)",border:"1px solid rgba(245,158,11,0.22)",fontSize:12,fontWeight:700}}>Menu</button>
              <button onClick={()=>setUser(null)} style={{color:"var(--muted)",display:"flex",padding:8,borderRadius:12,background:"rgba(255,255,255,0.68)",border:"1px solid var(--border)"}}>
