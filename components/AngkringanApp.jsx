...
const DayDetail = ({date, orders, expenses, kasirs, menus, onBack, businessDate}) => {
  ...
  const paid = orders.filter(o=>o.status==="paid"&&orderSessionDate(o)===date);
  const allExps = [...expenses.filter(e=>expenseDateKey(e)===date), ...addExpenses];
  const {pemasukan, pengeluaran, modalMitra, totalKeluar, kas} = calcFinanceSummary({orders: paid, expenses: allExps, menus});
