...
const MenuDrawer = ({open,onClose,items,screen,onNavigate,isOwner,onOpenTim,onOpenMenu,onLogout}) => {
  ...
  return(
    <div style={{position:"absolute",inset:0,zIndex:450,background:"rgba(15,23,42,0.34)",backdropFilter:"blur(4px)",display:"flex"}} onClick={onClose}>
      <div className="fu" style={{width:"min(86vw,320px)",height:"100%",background:"linear-gradient(180deg,#FFFFFF 0%,#F8FAFC 100%)",borderRight:"1px solid var(--border)",padding:"18px 14px 20px",display:"flex",flexDirection:"column",gap:14,boxShadow:"0 22px 50px rgba(15,23,42,0.18)"}} onClick={e=>e.stopPropagation()}>
  ...
  <div className="hdr-shell glass-card" style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,
    ...
    <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0,flex:1}}>
      ...
      <div style={{minWidth:0}}>
        <h2 className="sora" style={{fontSize:18,fontWeight:800,color:"var(--text)",letterSpacing:"-0.03em",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{title}</h2>
        {sub&&<p style={{fontSize:11,color:"var(--muted)",marginTop:3,fontWeight:500,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{sub}</p>}
      </div>
    </div>
    {right}
  </div>
...
const POS = ({menus,orders,setOrders,user,businessDate,currentSessionId,kasirs,setScreen}) => {
  ...
  if(step==="menu")return(<div className="pos-menu-screen" style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
    <div style={{padding:"11px 18px 9px",borderBottom:"1px solid var(--border)",flexShrink:0}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:showSearch?10:8}}>
        <div style={{minWidth:0}}>
          <p style={{color:"var(--amber)",fontWeight:700,fontSize:14,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{name}</p>
          {!showSearch&&<p style={{color:"var(--muted)",fontSize:11,marginTop:2}}>Pilih menu untuk ditambahkan ke pesanan</p>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          <button onClick={()=>{if(showSearch){setShowSearch(false);setSearch("");}else setShowSearch(true);}} style={{width:38,height:38,borderRadius:12,background:showSearch?"var(--amber-dim)":"rgba(255,255,255,0.82)",border:`1px solid ${showSearch?"rgba(245,158,11,0.28)":"var(--border)"}`,display:"flex",alignItems:"center",justifyContent:"center",color:showSearch?"var(--amber)":"var(--muted)",boxShadow:"0 10px 22px rgba(15,23,42,0.05)"}}>
      ...
      {showSearch&&(
        <div style={{display:"flex",alignItems:"center",gap:8,background:"var(--card2)",border:"1px solid var(--border)",borderRadius:11,padding:"8px 12px",marginBottom:9}}>
      ...
      {!showSearch&&quickMenus.length>0&&(<div style={{marginBottom:9}}>
...
export default function AngkringanApp() {
  ...
  const headerLeft = (
    <button onClick={()=>setNavOpen(true)} style={{width:40,height:40,borderRadius:12,background:"rgba(255,255,255,0.88)",border:"1px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--muted)",flexShrink:0}}>
      <svg width={19} height={19} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round"><path d="M4 7h16M4 12h16M4 17h16"/></svg>
    </button>
  );
