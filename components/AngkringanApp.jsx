...
const Dashboard = ({orders,expenses,setExpenses,user,setScreen,target,setTarget,kasirs,businessDate,sessionOpen,sessionDate,onBuka,onTutup}) => {
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:"16px 18px 12px"}}>
      ...
      {/* Form Pengeluaran — kasir bisa input */}
      {user.role==="kasir"&&sessionOpen&&(
        <div style={{marginTop:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
            ...
            <button onClick={()=>setShowExpForm(!showExpForm)} style={{
              background:"var(--amber)",color:"#fff",borderRadius:9,padding:"5px 12px",
          ...
          {showExpForm&&(
            <Card className="fi" style={{marginBottom:10,display:"flex",flexDirection:"column",gap:10}}>
              <TxtInput label="Keterangan" value={expDesc} onChange={setExpDesc} placeholder="Beli es batu, gula..."/>
              <TxtInput label="Jumlah" type="number" value={expAmt} onChange={setExpAmt} placeholder="50000" prefix="Rp"/>
              {expOk&&<p style={{color:"var(--green)",fontSize:12,textAlign:"center",fontWeight:600}}>✓ Pengeluaran tersimpan!</p>}
              <div style={{display:"flex",gap:8}}>
                <Btn v="ghost" onClick={()=>setShowExpForm(false)} sm full>Batal</Btn>
