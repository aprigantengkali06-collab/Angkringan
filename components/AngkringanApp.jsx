...
const SuccessOverlay = ({type,kembalian}) => (
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:9999,
    ...
    <div style={{width:120,height:120,borderRadius:"50%",
      background:type==="tambah"?"var(--amber)":"var(--green)",
      display:"flex",alignItems:"center",justifyContent:"center",
      animation:"popIn 0.45s cubic-bezier(0.175,0.885,0.32,1.275) both",
      boxShadow:type==="tambah"?"0 0 60px rgba(212,130,10,0.45)":"0 0 60px rgba(16,185,129,0.45)"}}>
    ...
    <p className="sora" style={{color:"#fff",fontWeight:800,fontSize:24,textAlign:"center",letterSpacing:"-0.5px"}}>
      {type==="lunas"?"Pembayaran Lunas! ✓":type==="nanti"?"Pesanan Tercatat! ✓":"Item Ditambahkan! ✓"}
    </p>
    {type==="lunas"&&Number(kembalian)>0&&(
      <div style={{background:"rgba(16,185,129,0.15)",border:"1px solid rgba(16,185,129,0.35)",
        borderRadius:16,padding:"14px 32px",textAlign:"center"}}>
    ...
    {(type==="lunas"||type==="nanti")&&(
      <div style={{display:"flex",alignItems:"center",gap:7}}>
        ...
        <p style={{color:"rgba(255,255,255,0.45)",fontSize:12}}>Mencetak struk customer...</p>
      </div>
    )}
  </div>
);
  ...
  const mitraStats=useMemo(()=>{
    ...
    paidToday.forEach(order=>{
      (order.items||[]).forEach(item=>{
        if(!item.mitraId) return;
        const menuRef=menus.find(m=>m.id===item.menuId);
        const hargaMitra=Number(menuRef?.hargaMitra)||0;
        if(!sm[item.mitraId]) sm[item.mitraId]={penjualan:0,modal:0};
        const qty=Number(item.qty)||0;
        const hargaJual=Number(item.price)||0;
        sm[item.mitraId].penjualan+=hargaJual*qty;
        sm[item.mitraId].modal+=hargaMitra*qty;
      });
    });
...
const POS = ({menus,orders,setOrders,user,businessDate,currentSessionId,kasirs,setScreen}) => {
  ...
  const addFromSheet=()=>{
    ...
    setCart(p=>{
      ...
      return [...p,{cartKey:key,menuId:m.id,name:displayName,price:m.price,qty:1,suhu,note}];
    });
  ...
  const submit=now=>{
    ...
    setSuccessState({type:"nanti",kembalian:0});
    printStruk(newOrder,0,kasirs,"nanti");
    setTimeout(()=>{setSuccessState(null);reset();},2200);
  };
  const konfirmasiBayar=()=>{
    ...
    setSuccessState({type:"lunas",kembalian:kemb});
    printStruk(newOrder,kemb,kasirs,"lunas");
    setTimeout(()=>{setSuccessState(null);reset();},2200);
  };
  ...
  if(successState)return <SuccessOverlay type={successState.type} kembalian={successState.kembalian}/>;
  if(step==="name")return(
...
const Tagihan = ({orders,setOrders,menus,user,kasirs,businessDate,currentSessionId}) => {
  ...
  const addItemFromSheet = () => {
    ...
    setOrders(prev=>prev.map(order=>{
      ...
      if(existingIndex>=0){
        ...
        items.push({cartKey:lineKey,menuId:m.id,name:displayName,price:m.price,qty:1,suhu,note});
      }
    ...
    setSheetNote("");
    setAdding(false);
    setSuccessState({type:"tambah",kembalian:0});
    setTimeout(()=>setSuccessState(null),1100);
  };
  ...
  const bayarSatuItem=()=>{
    ...
    if(isCompleted){
      ...
      const paidOrderForPrint = normalizeOrder({
        ...
        paidAt,
        kasirId:user.id,
        items:(ord.items||[]).filter(item=>getLineKey(item)===bayarItem.lineKey).map(item=>({...item,paid:true,cartKey:getLineKey(item)})),
        total:bayarItem.subtotal,
        lastDeviceId:user.id,
      });
      setSuccessState({type:"lunas",kembalian:kemb});
      printStruk(paidOrderForPrint,kemb,kasirs,"lunas");
      setTimeout(()=>setSuccessState(null),2200);
    } else {
      setSuccessState({type:"tambah",kembalian:0});
      setTimeout(()=>setSuccessState(null),1100);
    }
  ...
  const konfirmasiLunas=()=>{
    ...
    setSel(null);
    setSuccessState({type:"lunas",kembalian:kemb});
    printStruk(updatedOrder,kemb,kasirs,"lunas");
    setTimeout(()=>setSuccessState(null),2200);
  };
  ...
  return(<div style={{flex:1,overflowY:"auto",padding:"17px"}}>
    ...
    :openOrders.map(o=>(<div key={o.id} onClick={()=>setSel(o.id)} style={{background:"var(--card)",border:"1px solid var(--border)",
