...
const POS = ({menus,orders,setOrders,user,businessDate,currentSessionId,kasirs,setScreen}) => {
  ...
  const submit=now=>{
    ...
    setOrders(p=>[...p,normalizeOrder({id:genId("ORD"),customerName:name,status:"open",
      sessionDate:businessDate,sessionId:currentSessionId||null,createdAt:localISO(),paidAt:null,items:cart,total,kasirId:user.id,lastDeviceId:user.id})]);reset();
  };
  const konfirmasiBayar=()=>{
    if(!uangDibayar||parseInt(uangDibayar)<total)return;
    setOrders(p=>[...p,normalizeOrder({id:genId("ORD"),customerName:name,status:"paid",
...
const Tagihan = ({orders,setOrders,menus,user,kasirs,businessDate,currentSessionId}) => {
  ...
  const bayarSatuItem=()=>{
    ...
    setBayarItem(null);
    setUangItem("");
    const remainingOpen = (ord?.items||[]).filter(item=>getLineKey(item)!==bayarItem.lineKey && !item.paid);
  ...
  const konfirmasiLunas=()=>{
    ...
    const paidAt = localISO();
    setOrders(prev=>prev.map(order=>order.id===sel
      ? normalizeOrder({
          ...order,
          status:"paid",
          sessionDate: orderSessionDate(order) || businessDate,
          ...
          paidAt,
          kasirId:user.id,
          items:(order.items||[]).map(item=>({...item,paid:true,cartKey:getLineKey(item)})),
          lastDeviceId:user.id,
        })
      : order
    ));
