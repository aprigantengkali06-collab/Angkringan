...
const getOrderMitraModal = order => (order?.items||[]).reduce((sum,item)=>sum+(Number(item?.hargaMitra)||0)*(Number(item?.qty)||0),0);
const getOrdersMitraModal = orders => orders.reduce((sum,order)=>sum+getOrderMitraModal(order),0);
const fmtWaktu = pa => {
  if(!pa) return "";
...
const printDayPDF = (date, orders, expenses, kasirs) => {
  ...
  const modalMitra = getOrdersMitraModal(paid);
...
const printMonthPDF = (monthKey, monthLabel, orders, expenses, kasirs) => {
  ...
  const dayMap = {};
  paid.forEach(o=>{const d=orderSessionDate(o);if(!d) return; if(!dayMap[d])dayMap[d]={pemasukan:0,pengeluaran:0,modalMitra:0,orders:[]};dayMap[d].pemasukan+=o.total;dayMap[d].modalMitra+=getOrderMitraModal(o);dayMap[d].orders.push(o);});
  exps.forEach(e=>{if(!e.date) return; if(!dayMap[e.date])dayMap[e.date]={pemasukan:0,pengeluaran:0,modalMitra:0,orders:[]};dayMap[e.date].pengeluaran+=e.amount;});
...
const Keuangan = ({orders, expenses, setExpenses, kasirs, businessDate}) => {
  ...
  const getMonthSummary = (monthKey) => {
    ...
    const modalMitra = getOrdersMitraModal(paid);
    const totalKeluar = pengeluaran + modalMitra;
    return {pemasukan, pengeluaran, modalMitra, totalKeluar, kas: pemasukan-totalKeluar};
  };
  const getDaySummary = (ds) => {
    const paid = orders.filter(o=>o.status==="paid"&&orderSessionDate(o)===ds);
    const pemasukan = paid.reduce((s,o)=>s+o.total,0);
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 18px"}}>
        ...
        /* Level 2: Month Detail */
          ...
          <div className="fi">
            ...
            {/* Month summary */}
            {(()=>{const {pemasukan,totalKeluar,kas}=getMonthSummary(selMonth.key);return(
...
const Dashboard = ({orders,expenses,setExpenses,user,setScreen,target,setTarget,kasirs,mitras,menus,businessDate,sessionOpen,sessionDate,onBuka,onTutup}) => {
  ...
  const totalPengeluaran = pengeluaran + totalMitraModal;
  const bersih = pemasukan - totalPengeluaran;
