...
const orderSessionDate = order => dateKey(order?.createdAt) || paidAtDate(order?.paidAt);
...
const printMonthPDF = (monthKey, monthLabel, orders, expenses, kasirs) => {
  ...
  const pengeluaran = exps.reduce((s,e)=>s+e.amount,0);
  const kas = pemasukan - pengeluaran;
  // Group by day (selalu pakai YYYY-MM-DD supaya tidak Invalid Date)
  const dayMap = {};
  paid.forEach(o=>{const d=orderSessionDate(o);if(!d) return; if(!dayMap[d])dayMap[d]={pemasukan:0,pengeluaran:0,orders:[]};dayMap[d].pemasukan+=o.total;dayMap[d].orders.push(o);});
  exps.forEach(e=>{if(!e.date) return; if(!dayMap[e.date])dayMap[e.date]={pemasukan:0,pengeluaran:0,orders:[]};dayMap[e.date].pengeluaran+=e.amount;});
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        ...
        <button onClick={()=>printDayPDF(date,orders,[...expenses,...addExpenses],kasirs)}
...
const Keuangan = ({orders, expenses, setExpenses, kasirs, businessDate}) => {
  ...
  const allDates = useMemo(()=>{
    const paidDates = orders.filter(o=>o.status==="paid").map(orderSessionDate).filter(Boolean);
  ...
  const getMonthSummary = (monthKey) => {
    const pemasukan = orders.filter(o=>o.status==="paid"&&orderSessionDate(o)?.startsWith(monthKey)).reduce((s,o)=>s+o.total,0);
  ...
  const getDaySummary = (ds) => {
    const pemasukan = orders.filter(o=>o.status==="paid"&&orderSessionDate(o)===ds).reduce((s,o)=>s+o.total,0);
...
const Dashboard = ({orders,expenses,setExpenses,user,setScreen,target,setTarget,kasirs,businessDate,sessionOpen,sessionDate,onBuka,onTutup}) => {
  ...
  const paidToday=activeDashboardDate?orders.filter(o=>o.status==="paid"&&orderSessionDate(o)===activeDashboardDate):[];
  const openOrders=orders.filter(o=>o.status==="open");
  const expsToday=activeDashboardDate?expenses.filter(e=>e.date===activeDashboardDate):[];
    ...
    Promise.all([
      ...
      supabase.from("settings").select("*").then(({data})=>{
        if(data) {
          ...
          const sd=data.find(r=>r.key==="session_date"); if(sd) setSessionDate(sd.value||null);
        }
      }),
