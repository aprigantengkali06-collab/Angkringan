...
// ── Dashboard ──
const Dashboard = ({orders,expenses,user,setScreen,target,setTarget,kasirs,businessDate,sessionOpen,sessionDate,onBuka,onTutup}) => {
  ...
  const pengeluaran=expenses.filter(e=>e.date===businessDate).reduce((s,e)=>s+e.amount,0);
...
// ── POS ──
const POS = ({menus,orders,setOrders,user,businessDate}) => {
    ...
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"var(--bg)",maxWidth:480,margin:"0 auto",position:"relative",overflow:"hidden"}}>
      ...
      {!overlay&&(<>
        ...
        <div key={screen} className="fi" style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          {screen==="home"&&<Dashboard orders={orders} expenses={expenses} user={user} setScreen={setScreen} target={target} setTarget={setTarget} kasirs={kasirs} businessDate={businessDate} sessionOpen={sessionOpen} sessionDate={sessionDate} onBuka={handleBuka} onTutup={handleTutup}/>}
          {screen==="pos"&&<POS menus={menus} orders={orders} setOrders={setOrders} user={user} businessDate={businessDate}/>}
