    ...
    <div className="pos-name-screen" style={{flex:1,overflowY:"auto",padding:"18px 18px 12px",display:"flex",flexDirection:"column",gap:16}}>
      ...
      {todayOpen.length>0&&(
        <div>
          ...
          {todayOpen.map((o,i)=>(
            <div key={o.id} onClick={()=>setScreen("tagihan")} style={{background:"var(--card)",border:"1.5px solid rgba(239,68,68,0.18)",
              borderRadius:14,padding:"11px 13px",marginBottom:8,cursor:"pointer",
              boxShadow:"0 2px 10px rgba(239,68,68,0.07)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{width:24,height:24,borderRadius:7,background:"rgba(239,68,68,0.10)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,color:"var(--red)",flexShrink:0}}>{i+1}</span>
                  <div>
                    <p style={{color:"var(--text)",fontWeight:700,fontSize:14}}>{o.customerName}</p>
                    <PaymentMeta order={o}/>
                  </div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0,marginLeft:8}}>
                  <span style={{color:"var(--red)",fontWeight:800,fontSize:13}}>{rupiah(o.total)}</span>
              ...
              <p style={{color:"var(--muted)",fontSize:11,marginTop:5,paddingLeft:32,lineHeight:1.4}}>
                {o.items.map(it=>`${it.name} ×${it.qty}${it.note?` • ${it.note}`:""}`).join(" · ")}
              </p>
              {kasirs.length>1&&<div style={{paddingLeft:32,marginTop:4}}><KasirChip kasirId={o.kasirId} kasirs={kasirs}/></div>}
            </div>
          ))}
        </div>
      )}
