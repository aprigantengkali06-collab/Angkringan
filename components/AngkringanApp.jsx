...
const buildNativeReceiptPayload = (order, kembalian, kasirs, mode="lunas", waktu=null) => {
  const kasir = kasirs.find(k=>k.id===order.kasirId);
  return {
    shopName: "ANGKRINGAN.",
    title: "Struk Pembayaran Customer",
    orderId: order?.id || "",
    ...
    mode,
    total: Number(order?.total)||0,
    dibayar: mode==="lunas" ? (Number(order?.total)||0) + (Number(kembalian)||0) : 0,
    ...
    footer: mode === "lunas" ? "Terima kasih sudah mampir! ☕" : "Mohon lunasi tagihan Anda",
  };
};
...
// ── Struk Customer ──
const printStruk = async (order, kembalian, kasirs, mode="lunas") => {
  const kasir = kasirs.find(k=>k.id===order.kasirId);
  const now = new Date();
  const waktu = now.toLocaleString("id-ID",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"});
  const nativePrinted = await tryNativeReceiptPrint(buildNativeReceiptPayload(order, kembalian, kasirs, mode, waktu));
...
const CatBar = ({val,set,categories=DEFAULT_FILTER_CATS}) => (
...
const DataTools = ({busy,onClose,onBackup,onRestore,onReset}) => {
      ...
      <div className="fu" style={{width:"min(100%, 480px)",maxHeight:"min(86vh, 760px)",overflowY:"auto",background:"linear-gradient(180deg,#FFFFFF 0%,#F8FAFC 100%)",border:"1px solid var(--border)",borderRadius:24,padding:18,boxShadow:"0 24px 60px rgba(15,23,42,0.20)",display:"flex",flexDirection:"column",gap:14}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
          <div>
            ...
            <p className="sora" style={{fontSize:22,fontWeight:800,color:"var(--text)",marginTop:4}}>Backup & Reset Data</p>
            <p style={{fontSize:12,color:"var(--muted)",lineHeight:1.6,marginTop:6}}>Simpan cadangan data ke file JSON, pulihkan lagi kapan saja, atau kosongkan transaksi tanpa perlu buka SQL Editor.</p>
          </div>
        ...
        <Card style={{display:"flex",flexDirection:"column",gap:12}}>
          <div>
            ...
            <p style={{fontSize:12,color:"var(--muted)",lineHeight:1.6,marginTop:4}}>Backup menyimpan menu, kasir, mitra, transaksi, pengeluaran, target, dan password owner ke satu file <strong>.json</strong>.</p>
          </div>
        ...
        <Card style={{display:"flex",flexDirection:"column",gap:12,border:"1px solid rgba(239,68,68,0.16)",background:"linear-gradient(180deg, rgba(255,255,255,0.98) 0%, rgba(254,242,242,0.98) 100%)"}}>
          <div>
            ...
            <p style={{fontSize:12,color:"var(--muted)",lineHeight:1.6,marginTop:4}}>Menghapus <strong>menu, transaksi, pengeluaran, dan sesi</strong>, tetapi tetap menyimpan data kasir, mitra, target, dan password owner.</p>
          </div>
...
export default function AngkringanApp() {
  ...
  const handleBackupDownload = () => {
    try{
      ...
      triggerJsonDownload(`backup-angkringan-${stamp}.json`, payload);
      alert("Backup berhasil diunduh. Simpan file JSON ini di tempat aman.");
    }catch(err){
  ...
  const handleRestoreBackup = async file => {
    ...
    try{
      ...
      await upsertMany("orders", nextOrders.map(order=>toDbOrder({...order, sessionId:null}, deviceId)));
      ...
      await loadFromSupabase();
      setOverlay(null);
      alert("Backup berhasil dipulihkan. Silakan cek menu, tim, dan laporan Anda.");
    }catch(err){
  ...
  return(<><FontStyle/>
    <div className="app-shell">
      ...
      {overlay==="data"&&<DataTools busy={dataBusy} onClose={()=>!dataBusy&&setOverlay(null)} onBackup={handleBackupDownload} onRestore={handleRestoreBackup} onReset={handleResetRingan}/>}
