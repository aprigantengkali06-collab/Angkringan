            ...
            return(<>
              ...
              {total_pages>1&&(
                <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:8,marginTop:8,marginBottom:4}}>
                  ...
                  <button onClick={()=>setPg(p=>Math.min(total_pages-1,p+1))} disabled={pg===total_pages-1}
                    style={{width:30,height:30,borderRadius:8,background:"var(--card2)",border:"1px solid var(--border)",
...
const POS = ({menus,orders,setOrders,user,businessDate,kasirs,setScreen}) => {
  ...
  const konfirmasiBayar=()=>{
    ...
    setOrders(p=>[...p,{id:genId("ORD"),customerName:name,status:"paid",
      createdAt:businessDate,paidAt:new Date().toISOString(),items:cart,total,kasirId:user.id}]);
...
const Tagihan = ({orders,setOrders,menus,user,kasirs,businessDate}) => {
  ...
  const bayarSatuItem=()=>{
    ...
    setOrders(p=>p.map(o=>{
      ...
      return {...o,items,total:sisaTotal,
        status:semuaLunas?"paid":"open",
        paidAt:semuaLunas?new Date().toISOString():null,
  ...
  const konfirmasiLunas=()=>{
    ...
    setOrders(p=>p.map(o=>o.id===sel?{...o,status:"paid",paidAt:new Date().toISOString(),kasirId:user.id,
      items:o.items.map(i=>({...i,paid:true})),total:0}:o));
